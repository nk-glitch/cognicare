...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void _loadNotifications(String userId) {
    if (widget.isCaretaker) {
      _listenForCaretakerAlerts(userId);
    } else {
      _listenForConnectionRequests(userId);
    }
  }

  // Real-time listener for caretaker missed-reminder alerts
  void _listenForCaretakerAlerts(String userId) {
    _firestore
      ...
      // Mark unread alerts as read in Firestore BEFORE setState so the
      // resulting snapshot already has isRead: true — prevents the flicker
      // where items briefly render as unread then immediately flip to read.
      ...
      final newNotifications = snapshot.docs.map((doc) {
        ...
        return NotificationItem.missedReminder(
          ...
          // Always treat as read locally — we just marked them all above.
          // This prevents the unread->read flash caused by the Firestore
          // write triggering a second snapshot.
          isRead: true,
        );
      }).toList();
      ...
      // Await writes after setState so the UI is not blocked
      if (writeFutures.isNotEmpty) await Future.wait(writeFutures);
    }, onError: (e) {
      print('Error listening for caretaker alerts: $e');
    });
  }

  // Real-time listener for patient connection requests
              ...
              Align(
                ...
                child: TextButton.icon(
                  ...
                  style: TextButton.styleFrom(
                    foregroundColor: const Color(0xFF7A6A70),
                  ),
                ),
              ),
            ],
